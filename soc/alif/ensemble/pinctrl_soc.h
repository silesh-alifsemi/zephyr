 ...
 * bits 0:2   [FFF] - Alternate function (0-7)
 * bits 3:5   [PPP] - Pin number within port (0-7)
 * bits 6:10  [PPPPP] - Port number (0-17, port 15 is LPGPIO)
 * bits 11:15 - Reserved
...
#define ALIF_PINCTRL_DRIVE_STRENGTH_CFG(node_id)				\
	(DT_PROP_OR(node_id, drive_strength, 4) == 2 ? 0 :                      \
	 DT_PROP_OR(node_id, drive_strength, 4) == 4 ? 1 :                      \
	 DT_PROP_OR(node_id, drive_strength, 4) == 8 ? 2 :                      \
	 DT_PROP_OR(node_id, drive_strength, 4) == 12 ? 3 : 1)
