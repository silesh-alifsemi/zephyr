 ...
 * TIMER_CLKSEL register (SEL1 at bit 0, SEL2 at bit 4, SEL3 at bit 8 and
 * SEL4 at bit 12). Only the two SoC-internal sources are described here:
 * S32K_CLK (selector value 0, the reset default) and 128K_CLK (value 1).
 * The remaining selector values route an external clock from the
 * LPTMRn_CLK_IO pin or cascade a channel with its even-numbered neighbour.
 *
 * The channels have no clock enable bit, so these clocks are always on and
 * only the source selection is programmed.
 *
 * @note LPTIMER2 and LPTIMER3 are absent on the E1C series.
 *
 * @{
 */
...
/** LPTIMER2 clock sourced from S32K_CLK */
#define ALIF_LPTIMER2_S32K_CLK      \
	ALIF_CLK_CFG(VBAT, TIMER_CLKSEL, 0U, 0U, 0U, 2U, 8U, ALIF_PARENT_CLK_S32K)
/** LPTIMER2 clock sourced from 128K_CLK */
#define ALIF_LPTIMER2_128K_CLK      \
	ALIF_CLK_CFG(VBAT, TIMER_CLKSEL, 0U, 0U, 1U, 2U, 8U, ALIF_PARENT_CLK_128K)
/** LPTIMER3 clock sourced from S32K_CLK */
#define ALIF_LPTIMER3_S32K_CLK      \
	ALIF_CLK_CFG(VBAT, TIMER_CLKSEL, 0U, 0U, 0U, 2U, 12U, ALIF_PARENT_CLK_S32K)
